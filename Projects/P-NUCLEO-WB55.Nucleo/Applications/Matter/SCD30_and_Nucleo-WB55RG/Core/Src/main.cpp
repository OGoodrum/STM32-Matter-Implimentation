...
I2C_HandleTypeDef hi2c1; /**< I2C1 handler for CO2 sensor (PB8=SCL, PB9=SDA) */

osThreadId_t SensorHandle;
extern const osThreadAttr_t SensorTask_attr = {
    .name = "SensorTask",
    .stack_size = 512 * 4,  // Increased for float printf support
...
void SensorTaskMain(void *argument);
/* USER CODE BEGIN PFP */
...
int main(void) {
	...
	MX_RNG_Init();
	Init_RTC();
	/* I2C1 initialisation for CO2 sensor (PB8=SCL, PB9=SDA) */
	MX_I2C1_Init();
	osKernelInitialize();
	MX_GPIO_Init();
	/* IPCC initialisation */
...
/**
 * @brief I2C1 Initialization Function (PB8=SCL, PB9=SDA)
 * @param None
 * @retval None
 */
static void MX_I2C1_Init(void) {
...
static void MX_GPIO_Init(void) {
	...
	GPIO_InitTypeDef GPIO_InitStruct = { 0 };
		...
		/* Configure GPIOs for LEDs bar shield */

		/**GPIO Configuration for instance 0
		 * PC1     ------> clock
		 * PC5     ------> data
		 */
		...
		/**GPIO Configuration for instance 1
		 * PD14     ------> clock
		 * PD12     ------> data
		 */
		...
		/**GPIO Configuration for instance 2
		 * PB10    ------> clock
		 * PB4     ------> data
		 * NOTE: PB8 removed - now used for I2C1_SCL
		 */
...
void SensorTaskMain(void *argument) {
    ...
    APP_DBG("SensorTask: Starting...");

    // Give sensor time to power up
    osDelay(2000);

    // Initialize the SCD30 driver with the I2C address (REQUIRED before any other scd30 calls)
    scd30_init(SCD30_I2C_ADDR_61);

    APP_DBG("SensorTask: Calling scd30_start_periodic_measurement...");
    error = scd30_start_periodic_measurement(0);  // 0 = no pressure compensation
    if (error != 0) {
        APP_DBG("SensorTask: scd30_start_periodic_measurement failed: %d", error);
    } else {
        APP_DBG("SensorTask: Sensor initialized OK");
    }

    while (1) {
        // Wait for measurement (SCD30 measures every 2 seconds by default)
        osDelay(2000);

        // Read sensor
        error = scd30_blocking_read_measurement_data(&co2, &temperature, &humidity);
        if (error == 0) {
            // Use integer conversion to avoid heap allocation from float printf (dtoa)
            int co2_int = (int)co2;
            ...
            //APP_DBG("CO2: %d ppm, Temp: %d.%d C, Humidity: %d.%d%%",
                    //co2_int, temp_int, temp_dec, hum_int, hum_dec);

            // Post sensor reading to AppTask so it can update the Matter cluster
            // on the correct thread context
            AppEvent sensor_event;
            sensor_event.Type = AppEvent::kEventType_Sensor;
            sensor_event.SensorEvent.CO2ppm          = (int16_t)co2_int;
            sensor_event.SensorEvent.TemperatureCx10 = (int16_t)((int)temperature * 10 + (temp_dec));
            sensor_event.SensorEvent.HumidityPctx10  = (int16_t)((int)humidity * 10 + (hum_dec));
            sensor_event.Handler = AppTask::SensorUpdateEventHandler;
            GetAppTask().PostEvent(&sensor_event);
        } else {
            APP_DBG("SensorTask: Read failed with error %d", error);
        }
    }
