...
CHIP_ERROR AppTask::StartAppTask() {
    ...
    extern void SensorTaskMain(void *argument);
    SensorHandle = osThreadNew(SensorTaskMain, NULL, &SensorTask_attr);
...
CHIP_ERROR AppTask::Init() {
    ...
    if (chip::Server::GetInstance().GetFabricTable().FabricCount() == 0) {
        ...
        APP_BLE_Init_Dyn_3();
        CHIP_ERROR error = KeyValueStoreMgr().Get(STM32ThreadDataSet, datasetBytes,
...
/**
 * Write sensor data to three Matter clusters on Endpoint 1:
 *   - TemperatureMeasurement (0x0402)
 *   - RelativeHumidityMeasurement (0x0405)
 *   - CarbonDioxideConcentrationMeasurement (0x040D)
 */
void AppTask::UpdateCO2ClusterState(int16_t co2ppm, int16_t temperatureCx10, int16_t humidityPctx10)
{
    // Temperature (0x0402): convert 0.1 C -> 0.01 C
    int16_t tempHundredths = temperatureCx10 * 10;
    EmberAfStatus status = Clusters::TemperatureMeasurement::Attributes::MeasuredValue::Set(
        STM32_LIGHT_ENDPOINT_ID, tempHundredths);
    if (status != EMBER_ZCL_STATUS_SUCCESS) {
        ChipLogError(NotSpecified, "ERR: updating Temperature %x", status);
    }

    // Humidity (0x0405): convert 0.1 % -> 0.01 %
    uint16_t humHundredths = static_cast<uint16_t>(humidityPctx10 * 10);
    status = Clusters::RelativeHumidityMeasurement::Attributes::MeasuredValue::Set(
        STM32_LIGHT_ENDPOINT_ID, humHundredths);
    if (status != EMBER_ZCL_STATUS_SUCCESS) {
        ChipLogError(NotSpecified, "ERR: updating Humidity %x", status);
    }

    // CO2 (0x040D): write raw ppm via emberAfWriteAttribute (no SDK Accessors for this cluster)
    status = emberAfWriteAttribute(
        STM32_LIGHT_ENDPOINT_ID,
        0x040D,   // CarbonDioxideConcentrationMeasurement
        0x0000,   // MeasuredValue
        reinterpret_cast<uint8_t *>(&co2ppm),
        ZAP_TYPE(INT16S));
    if (status != EMBER_ZCL_STATUS_SUCCESS) {
        ChipLogError(NotSpecified, "ERR: updating CO2 %x", status);
    }

    ChipLogProgress(NotSpecified, "Sensor: CO2=%dppm Temp=%d.%dC Hum=%d.%d%%",
        (int)co2ppm,
        temperatureCx10 / 10, temperatureCx10 % 10,
        humidityPctx10 / 10, humidityPctx10 % 10);
}

/**
 * Event handler: dispatched on the AppTask thread when SensorTask
 * posts a new reading.  Unpacks the SensorEvent fields and forwards
 * them to UpdateCO2ClusterState().
 */
void AppTask::SensorUpdateEventHandler(AppEvent * aEvent)
{
    if (aEvent->Type != AppEvent::kEventType_Sensor) {
        return;
    }
    sAppTask.UpdateCO2ClusterState(
        aEvent->SensorEvent.CO2ppm,
        aEvent->SensorEvent.TemperatureCx10,
        aEvent->SensorEvent.HumidityPctx10);
}
